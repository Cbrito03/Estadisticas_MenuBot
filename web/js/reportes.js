...
function carga_pais()
{
	var request = 'pais/search';
	var result = sendRequest("GET", request);
	console.log(result);
    if(result.status === 200 && result.responseText !== 'NOK')
    {
        for(var i in result.responseJSON)
        {
        	if(result.responseJSON[i].status)
        	{        	          
	            $('#select_pais').append($('<option>',
	            {
	                value: result.responseJSON[i].pais_id,
	                text : result.responseJSON[i].pais
	            }));
            } 
        }
